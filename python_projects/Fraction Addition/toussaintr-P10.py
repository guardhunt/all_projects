...
# Created:     Mar/30/2014
# ------------------------------------------------------------------------------
# Edited by: Roland Junior Toussaint
# ------------------------------------------------------------------------------
...
class Fraction:
    ...
    def add_fraction( self, fraction_to_add ):
        ...
        self.numerator = (self.numerator * fraction_to_add.denominator) + (fraction_to_add.numerator * self.denominator)        # Adding a fraction to "self". Then
                                                                                                                                # the numerator is multiplied by the
                                                                                                                                # denominator of the other function.
        self.denominator = self.denominator * fraction_to_add.denominator   # Denominators are multiplied together which results with the new denominator
...
def add_the_fractions(fraction1, fraction2):
    ...
    result = Fraction( fraction1.numerator, fraction1.denominator )
    # create a fraction object and set its attributes as the ones from fraction1
    result.add_fraction(fraction2)
    # Add a second fraction object to the original and change its value
    return result
...
def check_sum_fractions():
    ...
    from fraction_writer import output_correct
    # Importing output_correct to display on the screen when the user is right
    from fraction_writer import output_incorrect
    # Importing output_correct to display on the screen when the user is wrong
    actual_result = add_the_fractions(fraction1, fraction2)
    # The actual result containts the real value of the sum of fractions
    actual_result.reduce_fraction()
    # reduce the fraction
    fraction_result.reduce_fraction()
    # reduce the fraction


    # The following will check if the user's result is correct by comparing it with the result from " add_the_fractions "
    if (actual_result.numerator == fraction_result.numerator) and actual_result.denominator == fraction_result.denominator:
        output_correct()            # If the answer is right, print right

    else:
        output_incorrect()          # If answer is wrong, print wrong
