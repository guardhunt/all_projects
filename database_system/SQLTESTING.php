      ...
      <div class="jumbotron jumfix">
		...
		// ATTENDANCE QUERY
			...
			if($fq->num_rows != 0){
				...
				echo "<div class = 'table-responsive'>";
				echo "<table class = 'table-striped table-condensed tables' align='center' border=1><tbody>";
				echo "<tr><td align='center'><b>Faculty Name</b></td></tr>";
				while($row = $fq->fetch_array()){
					$BNumber = $row['bnumber'];
					$quick = $link->query("SELECT firstname, mi, lastname FROM faculty WHERE bnumber = '$BNumber'");
					$r = $quick->fetch_array();
					$name = $r['firstname']." ".$r['mi']." ".$r['lastname'];
					$fac_t = $fac_t."<tr><td align = 'center'>".$name."</td></tr>";
					$fac_num++;
				}
			...
			if($aq->num_rows != 0){
				...
				echo "<div class = 'table-responsive'>";
				echo "<table class = 'table-striped table-condensed tables' align='center' border=1><tbody>";
				echo "<tr><td align='center'><b>Activity Name</b></td></tr>";
				while($row = $aq->fetch_array()){
					$ID = $row['ID'];
					$quick = $link->query("SELECT Title FROM Activities_t WHERE ID = '$ID'");
					$r = $quick->fetch_array();
					$act_t = $act_t."<tr><td align = 'center'>".$r['Title']."</td></tr>";
					$act_num++;
				}
