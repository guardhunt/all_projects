...
<CENTER>
	<h1>Home Directory</h1>
      <div class="jumbotron">
		...
		if($_POST['mi'] != NULL){
			$query = "INSERT INTO faculty VALUES ('$bnumber', '$title', '$firstname', '$mi', '$lastname', '$email', '$cpo',
			'$pro', '$ethnicity', '$gender', '$status', '$rank')";
			$add = $link -> query($query);
		}
		else{
			$query = "INSERT INTO faculty (bnumber, title, firstname, lastname, email, cpo, program, ethnicity, gender, status, rank) VALUES('$bnumber', '$title', '$firstname', '$lastname', '$email', '$cpo',
			'$pro', '$ethnicity', '$gender', '$status', '$rank')";
			$add = $link -> query($query);
		}
		if (!$add)
